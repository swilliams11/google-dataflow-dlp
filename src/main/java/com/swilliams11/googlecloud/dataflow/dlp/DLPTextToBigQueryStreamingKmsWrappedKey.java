package com.swilliams11.googlecloud.dataflow.dlp;
...
/**
 * Created this class in just in case I needed to modify the code to get the KMS wrapped key to work properly.
 * It turns out that there is a bug on the KMS side and this will not be necessary.
 */
public class DLPTextToBigQueryStreamingKmsWrappedKey {
    public static final Logger LOG = LoggerFactory.getLogger(DLPTextToBigQueryStreamingKmsWrappedKey.class);
    ...
    static class DLPTokenizationDoFn extends DoFn<KV<String, Table>, KV<String, Table>> {
        ...
        public void processElement(ProcessContext c) {
            ...
            LOG.info("Processing value ", nonEncryptedData.toString());
            ContentItem tableItem = ContentItem.newBuilder().setTable(nonEncryptedData).build();
            this.requestBuilder.setItem(tableItem);
            LOG.debug("Deidentify Template Name: ", this.requestBuilder.getDeidentifyTemplateName());
            LOG.debug("Deidentify Config: ", this.requestBuilder.getDeidentifyConfig());
            LOG.debug("DLP Request", this.requestBuilder.build()); // I think this is failing should be wrapped in try catch bloc
            DeidentifyContentResponse response =
                    dlpServiceClient.deidentifyContent(this.requestBuilder.build());
            LOG.debug("DLP Request sent; the response is: ", response);
