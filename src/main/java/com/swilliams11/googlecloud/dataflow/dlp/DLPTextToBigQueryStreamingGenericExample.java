...
public class DLPTextToBigQueryStreamingGenericExample {
    public static final Logger LOG = LoggerFactory.getLogger(DLPTextToBigQueryStreamingGenericExample.class);
                                                        ...
                                                        .forEach(
                                                                file -> {
                                                                    try (BufferedReader br = getReader(file)) {
                                                                        ...
                                                                        LOG.info("Added headers to map {}", headers.toString());
                                                                        c.output(KV.of(fileKey, headers));
    ...
    static class DLPTokenizationDoFn extends DoFn<KV<String, Table>, KV<String, Table>> {
        ...
        public void processElement(ProcessContext c) {
            ...
            try {
                ...
                LOG.info("The content table item is {}", tableItem.toString());
                this.requestBuilder.setItem(tableItem);
                LOG.info("Deidentify Template Name {}", this.requestBuilder.getDeidentifyTemplateName());
                LOG.info("Deidentify Config: {}", this.requestBuilder.getDeidentifyConfig());
                LOG.info("DLP Request is {}", this.requestBuilder.build()); // I think this is failing should be wrapped in try catch bloc
                DeidentifyContentResponse response =
                        dlpServiceClient.deidentifyContent(this.requestBuilder.build());
                LOG.info("DLP Request was send and the response is {}", response);
                tokenizedData = response.getItem().getTable();
